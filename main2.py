...
GCP_PROJECT_ID = os.getenv('MYSQL_DB')
...
app_etga.config["MYSQL_HOST"] = "localhost"
...
app_etga.secret_key = "parangaturimicuaro"

# CONFIGURACION CORREO ELECTRONICO

app_etga.config["MAIL_SERVER"] = "mail.kapps.me"
app_etga.config["MAIL_PORT"] = 587
app_etga.config["MAIL_USERNAME"] = "electronicatorres@kapps.me"
app_etga.config["MAIL_PASSWORD"] = "etga$@28"
remitente = "electronicatorres@kapps.me"
...
# BASE DE DATOS ETGA
# mysql = pymysql.connect("localhost","etga_usuario1","etga_usuario1xqi","etga")
...
def home():
    if "token" in session:
        ...
        # KAPP INFO
        cursor.execute(
            "select a.name name, k.name kapp, k.id id_kapp, k.clave clave, a.username from kapps_db.accounts a, kapps_db.kapps k where k.id=a.kapp_id and a.id = %s",
            [session["id"]],
        )
        account = cursor.fetchone()
        # DEFINE AMBIENTE SEGUN BASE DE DATOS
        if app_etga.config["MYSQL_DB"] == "etga_pruebas":
        ...
        cursor.execute(
            ...
            from alertas a left join alertas_conf acf on acf.id=a.id_alerta_conf \
                ...
                left join etga_clientes ec on ec.id=b.id_cliente \
...
def busqueda_cliente():
    ...
    if tipo_busqueda == 0:
        ...
        , ifnull(n_documento, "-") n_documento, ifnull(tipo_documento, "-") tipo_documento, tipo_cliente, c.fecha_ingreso, id_provincia, z.nombre \
            FROM etga_clientes c left join zonas z on z.id=c.id_zona WHERE c.id = %s \
                union all \
            ...
            FROM etga_clientes c2 left join zonas z2 on z2.id=c2.id_zona\
        ...
        cursor.execute(
            'select c.id, nombres, apellidos, telefono, telefono2, telefono3, ifnull(correo, "-") correo \
            , ifnull(n_documento, "-") n_documento, ifnull(tipo_documento, "-") tipo_documento, tipo_cliente, c.fecha_ingreso, id_provincia, z.nombre \
            FROM etga_clientes c left join zonas z on z.id=c.id_zona WHERE n_documento = %s \
...
def verificar_cliente():
    ...
    if datos is None:
        ...
        if tipo_accion == 1:
            ...
            cursor.execute(
                "INSERT INTO etga_clientes (nombres, apellidos, tipo_documento, n_documento, correo, telefono, telefono2, telefono3, tipo_cliente, id_provincia, id_zona, id_usuario) \
            ...
            cursor.execute(
                "SELECT max(id) codigo from etga_clientes where id_usuario = %s",
                [USUARIO],
            )
            datos = cursor.fetchone()
            cursor.close()
        ...
        if tipo_accion == 2:
            ...
            cursor.execute(
                "insert into etga_clientes_log  (id, nombres, apellidos , correo, telefono, telefono2, telefono3, tipo_cliente, id_provincia, id_zona, id_usuario_modificacion) \
                select id, nombres, apellidos , correo, telefono, telefono2, telefono3, tipo_cliente, id_provincia, id_zona, %s \
                from etga_clientes where id=%s",
                [USUARIO, id_cliente],
            )
            cursor.execute(
                "UPDATE etga_clientes set nombres=%s, apellidos=%s, correo=%s, telefono=%s, telefono2=%s, telefono3=%s, tipo_cliente=%s, id_provincia=%s, id_zona=%s \
                 where id=%s",
                [
                    nombres,
                    apellidos,
...
def cambio_estado():
    ...
    if tipo_accion == 9:  # RECHAZO
        if tipo_accion2 == 0:
            ...
            cursor.execute(
                "SELECT correo from etga_clientes where id = (select id_cliente from boletas where id_boleta=%s)",
                [id_boleta],
            )
            datos = cursor.fetchone()
            if datos is None:
...
def pdfs_mail():
    ...
    cursor.execute("SELECT correo from etga_clientes where id = %s", [id_cliente])
    datos = cursor.fetchone()
    cursor.close()
    if datos is None:
        return "NO-CORREO"
...
def boleta_pdf(id_boleta):
    ...
    pdf.set_font_size(9)
    pdf.multi_cell(w=0, h=1, txt="", align="J")
...
def cotizacion_pdf(id_boleta):
    ...
    pdf = MyFPDF(unit="mm", format=(73, 290))
    pdf.add_page()
    pdf.set_font("arial", size=10)
    boleta = datos_base.datos_boleta_impresion(mysql, id_boleta)
...
def comprobante_pdf(id_boleta):
    ...
    datos = cursor.fetchone()
    id_comprobante = datos["id"]
    pdf = MyFPDF(unit="mm", format=(73, 220))
    pdf.add_page()
    pdf.set_font("arial", size=10)
    boleta = datos_base.datos_boleta_impresion(mysql, id_boleta)
    fecha_split = datos["fecha"].split(" ")
...
def comprobante_venta_pdf(id_venta):
    ...
    cursor.execute(
        'select date_format(fecha,"%%Y-%%b-%%d %%h:%%i%%p") fecha, concat(c.nombres," ", c.apellidos) cliente, v.id_cliente, monto_sin_impuestos, impuestos, username \
            from venta_ventas v left join etga_clientes c on c.id=v.id_cliente \
    ...
    pdf = MyFPDF(unit="mm", format=(73, 145 + cursor.rowcount * 12))
    pdf.add_page()
    pdf.set_font("arial", size=10)
    ...
    for row in venta_contenido:
        ...
        pdf.ln(h=2)
    pdf.line(50, pdf.get_y(), 71, pdf.get_y())
    pdf.set_font_size(11)
    pdf.ln(h=4)
    pdf.multi_cell(
        w=0,
